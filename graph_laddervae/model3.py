...
class GraphVAE(nn.Module):
    ...
    def recover_adj_lower(self, l):
        ...
        adj = Variable(torch.zeros(self.max_num_nodes, self.max_num_nodes)).cuda()
    ...
    def pool_graph(self, x):
        if self.pool == 'max':
            out, _ = Variable(torch.max(x, dim=1, keepdim=False)).cuda()
        elif self.pool == 'sum':
            out = Variable(torch.sum(x, dim=1, keepdim=False)).cuda()
        return out
    ...
    def forward(self, input_features, adj):
        ...
        # graph_h = x
        print('h',graph_h)
        # graph_h = graph_h.view(-1, self.max_num_nodes * self.max_num_nodes)
        # vae
        # h_decode, z_mu, z_lsgms = self.vae(graph_h)
        kl_loss, h_decode = self.vae(graph_h)
        # print(kl_loss)
        # h_decode = torch.sum(h_decode,0,keepdim = True)
        # print(h_decode)
        out = F.sigmoid(h_decode)
        print(out)
        # print('out', out[0], len(out[0]))
        out_tensor = out.data
        ...
        adj_data = adj.data[0]
        print(' adj_data', adj_data.shape)
        # adj_data = Variable(adj_data).cuda()
        adj_features = torch.sum(adj_data, 1)
        # print('parameters',adj_data,recon_adj_tensor, adj_features,out_features)
        # print('shape',adj_data.shape,recon_adj_tensor.shape,adj_features.shape,out_features.shape)
        # S = self.edge_similarity_matrix(adj_data, recon_adj_tensor, adj_features, out_features,
        #         self.deg_feature_similarity)
        # print('S value', S,S.shape,S.norm(keepdim=True))
